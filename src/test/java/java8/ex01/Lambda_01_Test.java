...
public class Lambda_01_Test {
    ...
    private List<Person> filter(List<Person> persons, PersonPredicate predicate) {
    	...
    	List<Person> listMajeur = new ArrayList<Person>();
    	for (int i = 0; i < persons.size(); i++) {
			if (predicate.test(persons.get(i))){
				listMajeur.add(persons.get(i));
			}
		}
        return listMajeur;
    }
    ...
    public void test_filter_by_firstname() throws Exception {
        ...
        List<Person> result = filter(personList, p -> p.getFirstname().contentEquals("first_10"));
